...
def main():
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc='infer'):
            ...
            out = model(batch)
            probs = out['probs'].cpu().tolist()
            item_ids = batch['item_ids'].cpu().tolist()
            for item_id, prob in zip(item_ids, probs):
                rows.append({'row_id': rid, 'item_id': item_id, 'score': float(prob)})
                rid += 1

    ensure_dir(os.path.dirname(args.output) or '.')
    with open(args.output, 'w', newline='', encoding='utf-8') as f:
        w = csv.DictWriter(f, fieldnames=['row_id', 'item_id', 'score'])
        w.writeheader()
        w.writerows(rows)
